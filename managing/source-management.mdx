...
The backend already supports these source types, but they're not yet exposed in the **Add Source** picker UI. The corresponding doc pages describe the planned flow once the picker cards ship.

| Type | Status | Notes |
|------|--------|-------|
| [OneDrive](/integrations/onedrive) | Picker card pending | Microsoft Graph integration fully built; contact support to enable manually |
| [SharePoint](/integrations/sharepoint) | Picker card pending | Site → drive → file flow built; contact support to enable manually |
| Zendesk Help Center | Backend only | Ingestion code shipped; no UI yet |
| GitBook | Backend only | Ingestion code shipped; no UI yet |
